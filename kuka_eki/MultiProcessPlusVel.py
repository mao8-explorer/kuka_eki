...
# Define the client function for E1_eki_motion_client
def speed_client_process(speedControl_client):
    ...
    try:
        while speedControl_client._is_running:
        
            E1_cur_buffersize = speedControl_client.ReadBufferSize()
            if E1_cur_buffersize is not None and E1_cur_buffersize < E1_max_buffersize_limit:
                el_handle.e1 = e1_value
                speedControl_client.asyptp(el_handle)
    ...
    """
    机械臂速度独立控制, 配置ros_eki_vel.src文件一起运行
    """
if __name__ == "__main__":
    ...
    speedControl_process = multiprocessing.Process(target=speed_client_process, args=(speedControl_client,))

    
    e1_process.start()
    eki_process.start()
    speedControl_process.start()

    e1_process.join()
    eki_process.join()
    speedControl_process.join()
