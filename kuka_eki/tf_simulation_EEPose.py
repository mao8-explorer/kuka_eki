...
if __name__ == "__main__":
    ...
    target_pos = Pos(10.0, 1765.0, 1910.0, -40.0, 90.0, -93.0, e1 = init_el_value)
    eki_motion_client.ptp(target_pos, 50)
    T_a6_to_a7 = tf_to_pose() # TCP工具坐标系 get from rviz - based on URDF (Trust!)


    TCP_target_pos = Pos(190.0, 2000.0, 1134.0, 90.0, 180.0, 0.0, e1 = init_el_value)
    ##---------------->>>
    ...
    # 调用运动控制客户端
    eki_motion_client.ptp(Pos(*xyzabc, e1= init_el_value),50)
    ...
    try:
        while eki_motion_client._is_running:
            ...
            if cur_buffersize < max_buffersize_limit: 
                ...
                TCP_target_pos = Pos(x,y, 1134.0, 90.0, 180.0, rx, e1 = e1_value)
                # 坐标系转换接口！！
                ##---------------->>>
                TCP_pose = xyzabc_in_mm_deg_to_pose([*TCP_target_pos])
                ...
                # 调用运动控制客户端
                eki_motion_client.ptp(Pos(*xyzabc, e1= e1_value),50)
